---
title: "Exp analysis - exp20 and 21 - BanditStaticRegMonster"
...
img_path <- "~/Code/parkid/analysis/img/exp20-21/"
dir.create(img_path)
```
...
exp_names <- c("exp20", "exp21", "exp72")
agent_names <- c("ParKid", "ParPar", "Oracle")
file_names <-
  c(
    "par_action",
    "par_policy",
...
tmp %>% 
  ggplot(aes(x=agent, y=total_R)) +
  geom_boxplot(width=0.6, size=0.4, alpha=1, notch=TRUE) +
  theme_classic2() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  labs(x="", y="Total reward", tag="") -> p1
