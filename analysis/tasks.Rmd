...
classic %>% 
  ...
  labs(x="Action", y="Reward", title="Initial payout (40 trials)") +
...
# save
name <- "img/task1a"
ggsave(paste(name, ".pdf", sep=""), plot=p1, width = 1.2*2, height = 0.6*2)
```
...
classic %>% 
  ...
  scale_y_continuous(limits=c(0,1.1), breaks = c(0, 1)) +
  scale_fill_manual(values=c("darkgrey", "black")) +
  labs(x="Action", y="Reward", title="Changed (40 trials)") +
  facet_wrap(choice~., nrow=1) +
  theme_pubr(base_size = 6, legend = "none") +
  theme(strip.background = element_blank()) +
  theme(axis.text.x = element_blank()) +
...
print(p2)
name <- "img/task1b"
ggsave(paste(name, ".pdf", sep=""), plot=p2, width = 1.2*2, height = 0.6*2)
```
```{r, fig.width=1.2, fig.height=0.9}
p1 / p2
```